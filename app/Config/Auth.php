...
class Auth extends ShieldAuth
{
    ...
    {
        $session = session();
        $requestedUrl = $session->getTempdata('beforeLoginUrl');
        $url = is_string($requestedUrl) && $this->isAllowedPostLoginRedirect($requestedUrl)
            ? $requestedUrl
    ...
    private function isAllowedPostLoginRedirect(string $url): bool
    {
        $user = auth()->user();
        ...
        if ($user->inGroup('superadmin')) {
            return ! in_array($path, ['/login', '/logout', '/register'], true)
                && ! str_starts_with($path, '/login/')
                && ! str_starts_with($path, '/auth/');
        }

        if ($user->inGroup('teacher')) {
            return $path === '/teacher' || str_starts_with($path, '/teacher/');
        }
